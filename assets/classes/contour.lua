...
◘ Contour.apply(body, shapes, fixdef, filter) -- filter new 2019-10-27
	...
	'filter' is Box2D filter data for collisions (masks) new 2019-10-27
...
function Contour.clean(con, dist)
	...
	while l > 1 and pointsAreClose(con[l], con[1], distSqrd) do l = l - 1 end
	...
	while true do
		while i < l and pointsAreClose(pt, con[i+1], distSqrd) do i = i + 2 end
		local i2 = i
		while i < l and (pointsAreClose(con[i], con[i+1], distSqrd)
			or slopesNearColinear(pt, con[i], con[i+1], distSqrd)) do i = i + 1 end
		if i >= l then break
		elseif i == i2 then pt = con[i] out[k] = pt i, k = i + 1, k + 1
		end
	end
	
	if i <= l then out[k] = con[i]; k = k + 1 end
	if k > 2 and slopesNearColinear(out[k-2], out[k-1], out[1], distSqrd) then k = k - 1 end
	if k <= l then for n = k, #out do out[n] = nil end end
...
local function checkIntegrity(con)
	...
	for i = 1, l - 1 do
		for j = i + 1, l do
			if isSamePoint(con[i], con[j]) then errors[#errors+1] = {i, j} end
		end
	end
...
local function isClockwise(con)
	local n2 = 1
	for i = 2, #con do if con[n2][1] > con[i][1] then n2 = i end end
	...
	if w3 == 0 then return h3 < 0
	elseif w1 == 0 then return h1 >= 0
	else return h3/w3 < h1/w1
	end
end
...
local function getConcavesNum(con)
	local n = 0
	for i = 1, #con do if con[i][3] then n = n + 1 end end
	return n
end
...
local function isSkew(con, p1, p2)
	...
	for i = 1, l do
		if isSegCross(con[i], con[i+1], con[p1], con[p2]) then return false end
	end
	return true
end
...
local function split(con, p1, p2)
	...
	ringify(con1)
	for i = 1, #con1 do con1[i][3] = isConcave(con1[i-1], con1[i], con1[i+1]) end

	local con2 = {}
	for i = 1, p1 do con2[i] = {con[i][1], con[i][2]} end
	for i = p2, #con do con2[i-p2+1+p1] = {con[i][1], con[i][2]} end
	ringify(con2)
	for i = 1, #con2 do con2[i][3] = isConcave(con2[i-1], con2[i], con2[i+1]) end
